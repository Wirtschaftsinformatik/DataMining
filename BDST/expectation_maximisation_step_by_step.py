...
class GMM:
	...
	def predict(self,Y):
		...
		for m,c in (zip(self.mu,self.cov)):
			...
			ax2.scatter(m[0],m[1],c=colour[i],zorder=10,s=100)
			ax2.annotate(s='cluster {}'.format(i), xy= (m[0]+.5,m[1]+.5),color=colour[i], fontsize=24)
			i+=1
...
GMM = GMM(X=X,number_of_sources = 3, iterations=50)
GMM.run()
prediction = GMM.predict([[0.5,0.5]])
print(prediction)
